...
export class Estimate {
  ...
  private get operationFeeMumav() {
    return (
      (Number(this._milligasLimit) / 1000 + GAS_BUFFER) * MINIMAL_FEE_PER_GAS_MUMAV +
      Number(this.opSize) * MINIMAL_FEE_PER_BYTE_MUMAV
    );
  }
   ...
   */
  get minimalFeeMumav() {
    return this.roundUp(MINIMAL_FEE_MUMAV + this.operationFeeMumav);
  }
  ...
  get suggestedFeeMumav() {
    return this.roundUp(this.operationFeeMumav + MINIMAL_FEE_MUMAV * 2);
  }
